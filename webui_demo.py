...
def get_answer(query, vs_path, history, mode, score_threshold=VECTOR_SEARCH_SCORE_THRESHOLD,
    ...
    if mode == "Bing搜索问答":
        for resp, history in local_doc_qa.get_search_result_based_answer(
            ...
            history[-1][-1] += source
            yield history, ""
    elif mode == "智能服务" and vs_path is not None and os.path.exists(vs_path) and "index.faiss" in os.listdir(
            vs_path):
...
knowledge_base_test_mode_info = ("知识库查询模式"
    # "【注意】\n\n"
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    ...
                    vs_name = gr.Textbox(label="请输入新建知识库名称，仅支持字母数字",
                                         lines=1,
                                         interactive=True,
                                         visible=True)
                ...
                with vs_setting:
                    ...
                    query.submit(get_answer,
                                 [query, vs_path, chatbot, mode, score_threshold, vector_search_top_k, chunk_conent,
                                  chunk_sizes],
                                 [chatbot, query])
    with gr.Tab("模型配置", visible=False):
