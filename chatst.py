...
请在左侧切换模式，目前支持直接与 LLM 模型对话或基于本地知识库问答。\n\n
...
#   这个暂时无法避免，在配置普通的机器上暂时也无需考虑。每个session尽量保持相同的模型配置以避免冲突。
...
chat_box = st_chatbox(greetings=[init_message.format(default_vs=ST_CONFIG.defalut_vs),
                                 '模型已成功加载，可以开始对话，或从左侧选择模式后开始对话。\n\n尽量不要刷新页面，以免模型出错或重复加载。',
                                 ],
                      )
